    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        if(isset($_GET['id'])){
            if($fname && $Email && $date && $phone && $address){
                $id = $_GET['id'];
                $query = "update patients set full_name=?, email=?, date_of_birth=?,phone_number=?,address=?";
                $stm = mysqli_prepare($mysql,$query);
                mysqli_stmt_bind_param($stm,'isssss',$id,$fname,$Email,$date,$phone,$address);
        ...
        <main class="flex-1 p-8 overflow-auto">
            ...
            <div class="grid gap-6 mb-8">
                <div class="bg-gray-800 p-6 rounded-lg">
                    ...
                    if (isset($_GET['id'])) {
                        $id = $_GET['id'];
                        $query = "select * from patiens where id=$id";
